...
function applyFilters() {
  ...
  return state.items.filter((item) => {
    const text = [
      item.title_original, item.title_ko, item.summary_ko, item.source_name,
      item.category, item.source_type, ...(item.company_tags || []), ...(item.risk_tags || [])
    ].join(" ").toLowerCase();
...
function renderStats(items) {
  ...
  $("bcgCount").textContent = state.items.filter((i) => i.category === "BCG_GROUP_WATCH" || (i.company_tags || []).length).length;
...
function renderCards() {
  ...
  cards.innerHTML = items.map((item) => {
    ...
    return `
      <article class="card ${escapeHtml(item.priority)}">
        <div class="card-top">
          <div>
            <div class="meta">
            ...
            <h3><a href="${escapeHtml(item.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.title_ko || item.title_original)}</a></h3>
...
async function init() {
  ...
  $("searchInput").addEventListener("input", (e) => { state.filters.q = e.target.value; renderCards(); });
  $("categoryFilter").addEventListener("change", (e) => { state.filters.category = e.target.value; renderCards(); });
  $("priorityFilter").addEventListener("change", (e) => { state.filters.priority = e.target.value; renderCards(); });
  $("sourceTypeFilter").addEventListener("change", (e) => { state.filters.sourceType = e.target.value; renderCards(); });
