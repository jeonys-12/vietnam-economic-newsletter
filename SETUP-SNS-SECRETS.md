...
1. X Developer Console에서 Project와 App을 생성합니다.
2. App의 Bearer Token을 발급합니다.
3. GitHub 저장소의 `Settings → Secrets and variables → Actions`로 이동합니다.
4. `X_BEARER_TOKEN` 이름으로 Bearer Token을 저장합니다.
5. `Actions → Update Vietnam Economic Newsletter → Run workflow`를 실행합니다.

정상 실행 시 `data/sns.json`의 `channels.x.status`가 `active`로 바뀝니다. 토큰이 없으면 `not_configured`가 표시되며, 대시보드의 X 최신 게시글 검색 링크는 계속 사용할 수 있습니다. X Recent Search API는 최근 공개 게시글 범위와 계정의 API 이용한도 내에서 작동합니다.
